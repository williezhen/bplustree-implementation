...
// See BPlusNode.put.
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.isEmpty()){
            ...
            int indexToInsert = 0;
            for (; indexToInsert < keys.size(); indexToInsert++){
                if (key.compareTo(keys.get(indexToInsert))==0){
                    throw new BPlusTreeException("found duplicate key in leaf node");
                }else if (key.compareTo(keys.get(indexToInsert)) < 0){
                    break;
                }
            }
            ...
            if (keys.size() <= 2 * d){
                ...
                int ptr = d;
                for (; ptr < keys.size(); ptr++){
                    rightKeys.add(keys.get(ptr));
                    rightRids.add(rids.get(ptr));
                }
                ...
                sync();
                newNode.sync();
                return Optional.of(new Pair<>(rightKeys.get(0), newNode.page.getPageNum()));
            }
        }
    }
...
// See BPlusNode.bulkLoad.
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        int d = metadata.getOrder();
        int maxCapacity = (int) Math.ceil(2 * d * fillFactor);

        while (keys.size() < maxCapacity & data.hasNext()){
            Pair<DataBox, RecordId> nextPair = data.next();
            keys.add(nextPair.getFirst());
            rids.add(nextPair.getSecond());
        }

        if (data.hasNext()){//need to create a new leafnode
            List<DataBox> rightKeys = new ArrayList<>();
            List<RecordId> rightRids = new ArrayList<>();
            Pair<DataBox, RecordId> nextPair = data.next();

            rightKeys.add(nextPair.getFirst());
            rightRids.add(nextPair.getSecond());

            LeafNode nLeafNode = new LeafNode(metadata, bufferManager, rightKeys, rightRids, Optional.empty(), treeContext);
            rightSibling = Optional.of(nLeafNode.page.getPageNum());
            sync();
            nLeafNode.sync();
            return Optional.of(new Pair<>(rightKeys.get(0), nLeafNode.page.getPageNum()));
        }
        sync();
        return Optional.empty();
    }
