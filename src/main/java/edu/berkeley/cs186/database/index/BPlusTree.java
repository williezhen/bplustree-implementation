...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> tmpNode = root.put(key, rid);
        if (tmpNode.isPresent()){
            DataBox pushkey = tmpNode.get().getFirst();
            Long rightChild = tmpNode.get().getSecond();
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // the tree's root if the old root splits.
        while (data.hasNext()){
            Optional<Pair<DataBox, Long>> tmpNode = this.root.bulkLoad(data, fillFactor);
            if (tmpNode.isPresent()){
                DataBox pushkey = tmpNode.get().getFirst();
                Long rightChild = tmpNode.get().getSecond();
                Long leftChild = root.getPage().getPageNum();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private Iterator<RecordId> nextIter;

        private final Optional<DataBox> Key;

        public BPlusTreeIterator() {
            leafNode = root.getLeftmostLeaf();
            Key = Optional.empty();
            nextIter = leafNode.scanAll();
        }

        public BPlusTreeIterator(DataBox key) {
            leafNode = root.get(key);
            Key = Optional.of(key);
            nextIter = leafNode.scanGreaterEqual(key);
        }
        ...
        public boolean hasNext() {
          ...
          return (nextIter.hasNext() ||
                    leafNode.getRightSibling().isPresent());
        }
